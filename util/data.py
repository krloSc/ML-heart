...
class Data():
	...
	def corr_matrix(self,labels="All",save=False):
		...
		pd.plotting.scatter_matrix(self.data[labels.index[1:5]])
		if save==True:
	...
	def encoder(self,target="cp"):
		...
		encoded=cat_encoder.fit_transform(self.data[[target]])
		categories=[]
		for cat in cat_encoder.categories_[0]:
			categories.append("cp="+str(cat));
		encoded=pd.DataFrame(encoded,columns=categories,dtype=np.int8)
		self.data=self.data.drop(["cp"],axis=1)
